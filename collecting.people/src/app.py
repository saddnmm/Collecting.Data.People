...
import datetime
import os
from config import KZN_URL, XAB_URL
...
def get_info(url):
    ...
    if response.status_code == 200:
        ...
        path = os.path.join(os.path.dirname(__file__), 'static', f'{city_name}.data.json')
        with open(path, 'w', encoding='utf-8') as json_file:
            json.dump(data, json_file, ensure_ascii=False, indent=4)

        print({'message' : f'Данные успешно сохранены в файл "{path}".'})
    else:
